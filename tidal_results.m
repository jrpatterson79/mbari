...
% time = res.time(2:end);
% trim = find(time >= 5*44739.2); % Finding steady-periodic portion of signal
% % z = abs(data.x);
...
%     'MarkerSize', 12)
% hold on
% plot(abs(phasor_1), z, 'LineWidth', 2, 'Color', [0 0.4470 0.7410])
% % ax.XTick = [0:0.2:1];
% % xlim([0 1])
% ax.YDir = 'reverse';
...
atm_tide = 5000 * sin(2*pi*(time./86400)); % Atmospheric forcing signal
